...
public class treeapple : MonoBehaviour
{
    ...
    public void Start()
    {
        OBJ_treeapple = this;
        I_collection = 0;
      //  G_basket = GameObject.Find("basket");
        B_levelcomp = false;
    ...
    public void BUT_clickApple()
    {
        ...
        {
            EventSystem.current.currentSelectedGameObject.GetComponent<Rigidbody2D>().gravityScale = 0.5f;
            B_lerp = true;
            I_collection++;
            if (I_collection == 8)
            {
            ...
            Invoke("THI_enableColl", 200f * Time.deltaTime);
        }
        else
        {
            Debug.Log("WRONG");
